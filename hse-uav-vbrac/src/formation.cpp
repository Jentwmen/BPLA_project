...
//parameter - radius, parameter2 - arc_length - больше не используется, автоматически вычисляется. parameter3 - sector_angle
void formation::generate_points_sector() {
	...
	//scale_factor=arc_length/dist_sum;
...
void formation::setFormRotAngle(const double rotangle) {
	angle_rotation_form=rotangle; //old +=...*M_PI/180
}
