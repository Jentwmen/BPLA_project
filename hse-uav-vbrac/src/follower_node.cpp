...
//double cur_x=0, cur_y=0, cur_z=0; // текущие координаты
geometry_msgs::Point cur_pose; //текущая позиция
geometry_msgs::Point goal_pose; //заданная позиция
double tolerance=1; //допустимое отклонение от заданной точки
double cur_vx=0, cur_vy=0; //текущая скорость
size_t status=0; //0-nothing, 1-formation_alg, 2-vbrac_alg, 3-vbrac-surf???? текущий статус работы БЛА
...
//Поулчение текущей скорости
void velocityCallback(const geometry_msgs::TwistStamped::ConstPtr& msg) {
	cur_vx=msg->twist.linear.x;
	cur_vy=msg->twist.linear.y;
}

//Подсчет дистанции между текущей точкой и трубуемой точкой
...
//обработчик события смены позиции последователя
void poseCallback(const geometry_msgs::PoseStampedPtr &msg) {
	cur_pose=msg->pose.position;
	if(status==1&&euclidian_distance(cur_pose, goal_pose)<=tolerance&&cur_vx==0.0&&cur_vy==0.0) { //БЛА в области заданной точки
		//TODO: отправка сигнала лидеру о том, что БЛА достиг своей позиции (сигнал - id БЛА)
		//TODO: две нижних строчки перенести в другую функцию-обработчик. Эти действия будут выполняться, когда все БЛА займут свои позиции, и лидер даст команду занять общую высоту
		//z=leader_z;
...
//Устанавливает высоту лидера
//TODO: в будущем сменить на высоту первого БЛА формации
void sameHeight() {
	z=leader_z;
	isSendNextVelocity=true;
}
...
void handle_form(const enoga::enoga &enoga_value) {
	...
	//процесс перестановки можно попробовать синхронизировать. Чтобы сначала перестроился один дрон, потом другой, после получения сигнала от предыдущего. Или можно слать только сигналы по достижению точки
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber sub = nodeHandle.subscribe("/uav0/mavros/local_position/pose",
                                             10, leadposeCallback);
  ros::Subscriber follower_pos=nodeHandle.subscribe("/uav1/mavros/local_position/pose", 10, poseCallback);
  ...
  commands_to_leader=nodeHandle.advertise<enoga::follower>("/commands_to_leader", 10);
  ...
  set_mode_client =
      nodeHandle.serviceClient<mavros_msgs::SetMode>("/uav1/mavros/set_mode");
